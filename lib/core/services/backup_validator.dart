  ...
  void validateBackup(Map<String, dynamic> decoded, int expectedBackupVersion) {
    ...
    for (final p in products) {
      ...
      if (barcode != null && barcode.isNotEmpty) {
        // Simple normalization for check: trim and lowercase
      ...
      final cost = p['costPrice'] != null ? (p['costPrice'] as num).toDouble() : 0.0;
    ...
    for (final c in customers) {
      ...
      final balance = (c['outstandingBalance'] as num).toDouble();
      if (balance < 0) {
    ...
    for (final s in sales) {
      ...
      final paid = s['amountReceived'] != null ? (s['amountReceived'] as num).toDouble() : 0.0;
      if (total < 0 || paid < 0) {
    ...
    for (final s in sales) {
      final id = s['id'] as String;
      final headerTotal = (s['totalAmount'] as num).toDouble();
      final calculatedTotal = itemTotals[id] ?? 0.0;
      if ((headerTotal - calculatedTotal).abs() > 0.01) {
    ...
    final customerRemainingBalances = <String, double>{};
    for (final cr in creditRecords) {
      ...
      final amount = (cr['amount'] as num).toDouble();
      final paid = (cr['paidAmount'] as num).toDouble();
      ...
      if (!isVoided) {
        customerRemainingBalances[customerId] =
            (customerRemainingBalances[customerId] ?? 0.0) + remaining;
      }
    }
    ...
    for (final c in customers) {
      final id = c['id'] as String;
      final headerBalance = (c['outstandingBalance'] as num).toDouble();
      final calculatedBalance = customerRemainingBalances[id] ?? 0.0;
      if ((headerBalance - calculatedBalance).abs() > 0.01) {
        throw Exception(
    ...
    for (final cp in creditPayments) {
      ...
      final amount = (cp['amount'] as num).toDouble();
      if (amount <= 0) {
    ...
    for (final p in products) {
      final barcode = p['barcode'] as String?;
      if (barcode != null && barcode.isNotEmpty) {
        p['barcode'] = barcode.trim().toLowerCase();
      }
    }
    for (final c in customers) {
      final contact = c['contactNumber'] as String?;
