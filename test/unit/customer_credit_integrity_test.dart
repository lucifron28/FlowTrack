...
void main() {
  ...
  test(
    'customer identity: separate by same name, reject duplicate contact',
    () async {
      // 1. Two customers with the same name and different contacts are created separately
      final id1 = await database.createCustomer(
        name: 'Ate Joy',
        contactNumber: '09171112233',
      );
      final id2 = await database.createCustomer(
        name: 'Ate Joy',
      ...
      final id3 = await database.createCustomer(
        name: 'Ate Joy',
        contactNumber: null,
      );
      final id4 = await database.createCustomer(
        name: 'Ate Joy',
        contactNumber: null,
      );
  ...
  test(
    'payment reversal: soft reversal, rebuild customer balance, void credit sale',
    () async {
      ...
      final updatedPayments = await database
          .watchCreditPayments(customerId)
          .first;
      ...
      final updatedRecords = await database
          .watchCreditRecords(customerId)
          .first;
  ...
  test('migration: upgrade database from version 3 to 5', () async {
    // 1. Setup version 3 tables without reversal or expense-void columns.
    await database.customStatement('PRAGMA foreign_keys = OFF;');
    await database.customStatement('DROP TABLE IF EXISTS credit_payments;');
    await database.customStatement('DROP TABLE IF EXISTS expenses;');
    ...
    ''');
    await database.customStatement('''
      INSERT INTO expenses (id, category, amount, expense_date, created_at, updated_at)
      VALUES ('expense-1', 'Utilities', 200, 1715000000000, 1715000000000, 1715000000000);
    ''');

    // 3. Run migration from 3 to 5.
    final migrator = database.createMigrator();
    await database.migration.onUpgrade(migrator, 3, 5);

    // 4. Verify reversal and expense-void columns default properly.
    ...
    expect(expenses.single.isVoided, isFalse);
